...
import { ALL_DRUGS, DRUG_CLASSES, UNIQUE_DRUGS, searchByBrand } from '../data/drugClasses';
import DrugClassBrowse from '../components/DrugClassBrowse';
import { autosaveDrugSearch } from '../services/autosave';
// Optional: import { getAuth } from 'firebase/auth'; to pass real userId
...
// ─── Local search: Brand → Generic → Class/Indication ────────
function searchDrugsLocal(term) {
  if (!term || term.trim().length < 1) return [];
  const t = term.toLowerCase().trim();

  // Tier 0 — Brand name resolution (instant, highest priority)
  const brandMatches = searchByBrand(t);

  // Tier 1 — Generic name / class / subclass matches
  ...
  // Tier 2 — Indication / condition keyword matches
  ...
  // Merge with priority: brand → generic name → indication
  const seen = new Set();
  const merged = [];
...
// ─── AI search fallback — brand names, generics, conditions ──
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 600,
        system: `You are an expert clinical pharmacology database with knowledge of both generic and brand drug names worldwide (Nigeria/NAFDAC, UK, US, EU, Africa, WHO).

Your job: given ANY input (brand name, generic name, drug class, condition, indication, abbreviation, or partial name), return the matching GENERIC drug name(s).

Rules:
- ALWAYS resolve brand names to their generic equivalents. E.g. "Coartem" → "Artemether-Lumefantrine", "Panadol" → "Paracetamol", "Augmentin" → "Co-Amoxiclav".
- Return ONLY a valid JSON array of generic drug name strings.
- Maximum 20 entries. No preamble, no markdown backticks, no explanation.
- If the input is a brand name for ONE drug, return that drug plus related drugs in its class.

Example: Input "Coartem" → ["Artemether-Lumefantrine (Coartem)", "Artesunate (IV/IM)", "Chloroquine", "Quinine (IV)"]
Example: Input "Panadol" → ["Paracetamol (Acetaminophen)", "Ibuprofen", "Aspirin", "Tramadol"]
Example: Input "Augmentin" → ["Co-Amoxiclav (Amoxicillin-Clavulanate)", "Amoxicillin", "Piperacillin-Tazobactam"]`,
...
export default function DrugSearchPage() {
  ...
  const handleSearch = useCallback((val) => {
    ...
    // 🔥 Autosave local results immediately (fire-and-forget)
    if (local.length > 0) {
      autosaveDrugSearch({ query: val, results: local, aiUsed: false });
    }
      ...
      <div style={{ position: 'relative', maxWidth: 640, margin: '0 auto' }}>
        <div style={{
          ...
          <input
            ...
            placeholder="Search by generic name, brand name, condition, class…"
            style={{
        ...
        <div style={{ marginTop: 8, textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: "'Fira Code', monospace", letterSpacing: 1 }}>
          600+ drugs · 500+ brand names · 21 EMDEX categories · AI-powered
        </div>
      </div>
